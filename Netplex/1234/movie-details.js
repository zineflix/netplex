...
async function fetchMovies(category,rowId){
  ...
  try{
    ...
    (data.results||[]).forEach(movie=>{
      ...
      card.addEventListener('click',()=>window.location.href=`movie-details.html?movie_id=${movie.id}`);
...
async function fetchBanner(){
  ...
  try{
    ...
    safeOn(bannerPlay,'click',()=>window.location.href=`movie-details.html?movie_id=${movie.id}`);
...
// ==============================
function toggleSearchBar(){qs('.search-bar')?.classList.toggle('show');}
...
const MOVIE_ENDPOINTS=[
  ...
  {url:'https://mapple.rip/watch/movie/',name:'Server 26 Ads'}
];
...
function buildServerUrl(server,id){
  if(!server||!server.url||!id)return '';
  const base=`${server.url}${id}`;
  if(server.url.includes('screenscape.me/embed?tmdb='))return base;
  if(server.url.includes('rivestream.ru/watch?type=movie&id='))return base;
  if(server.url.includes('moviesapi.to/movie/'))return base;
  return base.includes('?')?`${base}&autoplay=true`:`${base}?autoplay=true`;
}
...
async function fetchMovieDetails(){
  ...
  try{
    ...
    if(iframeContainer&&movieIframe&&movieId&&MOVIE_ENDPOINTS.length){
    ...
    safeOn(document,'click',e=>{
      ...
      if(!serverDropdown.contains(e.target)&&!changeServerBtn.contains(e.target)){
...
function changeServer(index){
  ...
  const selectedServer=MOVIE_ENDPOINTS[index];
  ...
  movieIframe.setAttribute('allow','autoplay; fullscreen; picture-in-picture; encrypted-media');
  movieIframe.setAttribute('sandbox','allow-scripts allow-same-origin allow-forms allow-popups allow-presentation');

  const url=buildServerUrl(selectedServer,movieId);

  if(!url){
    console.error('Could not build server URL.');
    return;
  }

  console.log('Loading server:',selectedServer.name);
...
safeOn(document,'DOMContentLoaded',()=>{
  ...
  // COMMENTS
  // ==============================
  if(typeof getComments==='function')safeOn(window,'load',getComments);
...
function toggleFullscreen(){
  ...
  const isTV=/smart-tv|smarttv|appletv|googletv|hbbtv|netcast|viera|roku|dtv|firetv|aftb|afta|bravia|tizen|web0s|tv bro|tvbrowser|tv safari/.test(userAgent);

...
async function fetchMovies(category, rowId) {
  ...
  try {
    const data = await getJSON(
      `${baseUrl}${endpoint}${endpoint.includes('?') ? '&' : '?'}api_key=${apiKey}&language=en-US&page=1`
    );
...
async function fetchBanner() {
  ...
  try {
    ...
    const bannerPlay =
      banner.querySelector('.play-button') || byId('banner-play-btn');
  ...
  try {
        ...
        member.innerHTML = `
          <img
            src="${actor.profile_path
              ? imgUrl(actor.profile_path, 'w185')
    ...
    if (
      ...
      safeOn(downloadBtn, 'click', () => {
        ...
        // The download iframe now has no sandbox restrictions.
    ...
    if (iframeContainer && movieIframe) {
      ...
      movieIframe.src =
        `${MOVIE_ENDPOINTS[0].url}${movieId}?autoplay=true`;
    ...
    safeOn(changeServerBtn, 'click', (e) => {
      ...
      const isCurrentlyDisplayed =
        ...
        serverDropdown.classList.contains('show');
      ...
      const icon =
        changeServerBtn.querySelector('.dropdown-icon');
    ...
    function changeServer(index) {
      ...
      const selectedServer =
        MOVIE_ENDPOINTS[currentServerIndex];

      // ==============================
      // ENABLE SANDBOX FOR MOVIE PLAYER
      // ==============================
      if (movieIframe) {
      ...
      if (selectedServer.url.includes('?id=')) {
        ...
        selectedServer.url.includes('moviesapi.to/movie/')
      ) {
        url = `${selectedServer.url}${movieId}`;
      } else {
        url =
          `${selectedServer.url}${movieId}?autoplay=true`;
      }
      ...
      console.log(
        `Changed to server: ${selectedServer.name}, URL: ${url}`
      );
    }
    ...
    // CLOSE IFRAME
    // ==============================
    const closeIframeBtn =
      byId('close-iframe-btn');
...
document.addEventListener('click', (e) => {
  ...
  const changeServerBtn =
    byId('change-server-btn');

  const serverDropdown =
    byId('server-dropdown');
  ...
  if (
    ...
    if (
      ...
      (!serverControl ||
        !serverControl.contains(e.target)) &&
...
function toggleFullscreen() {
  const iframe =
    document.getElementById('movie-iframe');
...
function disableSandbox() {
  ...
  if (sandboxBtn) {
    sandboxBtn.classList.remove(
      'on'
    );

    sandboxBtn.classList.add(
      'off'
    );

    sandboxBtn.textContent =
      'Sandbox: OFF';
  }
